...
static void mk_rel_attr(NODE *node, RelAttr

&relAttr);

static int mk_agg_rel_attrs(NODE *list, int max, AggRelAttr relAttrs[]);

static void mk_agg_rel_attr(NODE *node, AggRelAttr

&relAttr);

static void mk_order_relattr(NODE *node, int

& order, RelAttr &relAttr);

static void mk_group_relattr(NODE *node, bool

& group, RelAttr &relAttr);
...
static void mk_value(NODE *node, Value
...
{
relAttr.
relName = node->u.RELATTR.relname;
relAttr.
attrName = node->u.RELATTR.attrname;
}
...
{
relAttr.
func = node->u.AGGRELATTR.func;
relAttr.
relName = node->u.AGGRELATTR.relname;
relAttr.
attrName = node->u.AGGRELATTR.attrname;
}
...
{
value.
type = node->u.VALUE.type;
switch (value.type) {
case
INT:
        value
.
data = (void *) &node->u.VALUE.ival;
break;
case
FLOAT:
        value
.
data = (void *) &node->u.VALUE.rval;
break;
case
STRING:
        value
.
data = (void *) node->u.VALUE.sval;
break;
}
}
